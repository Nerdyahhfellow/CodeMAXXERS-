...
(() => {
  ...
  function analyzeUrl(url) {
    ...
    try {
      ...
      // 7. Dangerous file extension
      if (DANGEROUS_EXTENSIONS.test(parsed.pathname)) {
        threats.push({ type: 'download', label: 'Dangerous Download', detail: 'This link downloads a potentially dangerous file type' });
      }
  ...
  // Per-page URL cache — avoids rescanning the same URL twice on one page
  const urlResultCache = new Map(); // url -> { safe, threats }
  // Track URLs currently being checked so concurrent calls dont double-scan
  const urlPending = new Map(); // url -> Promise

  // Queue for throttling GSB requests — max 2 at a time
  let gsbActiveCount = 0;
  const gsbQueue = [];

  function runGsbQueue() {
    while (gsbQueue.length > 0 && gsbActiveCount < 2) {
      const { url, resolve } = gsbQueue.shift();
      gsbActiveCount++;
      chrome.runtime.sendMessage({ type: 'SCAN_LINK', url }, response => {
        ...
        resolve({ safe: !response.unsafe, threats: response.threats || [] });
      });
    }
  }

  async function checkUrlFull(url) {
    // Already have result for this URL on this page
    if (urlResultCache.has(url)) return urlResultCache.get(url);

    // Already checking this URL — return the same promise
    if (urlPending.has(url)) return urlPending.get(url);

    const promise = (async () => {
      // 1. Local pattern check — instant, no network
      ...
      // 2. GSB check via background (background has its own persistent cache)
      const result = await new Promise(resolve => {
        gsbQueue.push({ url, resolve });
        runGsbQueue();
      });
      const final = { safe: result.safe, threats: result.threats || [] };
      urlResultCache.set(url, final);
      return final;
    })();

    urlPending.set(url, promise);
    promise.finally(() => urlPending.delete(url));
    return promise;
  }
  ...
  async function addBadgeToLink(link) {
    ...
    // Step 1: instant local check — show badge immediately if pattern match
    const localThreats = analyzeUrl(url);
    if (localThreats.length > 0) {
      injectBadgeStyles();
      const badge = createBadge('unsafe');
      const labels = localThreats.map(t => t.label).join(', ');
      badge.title = `Shield: UNSAFE — ${labels}`;
      try { insertBadge(link, badge); } catch { return; }
      link.style.outline = '1.5px solid #dc2626';
      link.style.outlineOffset = '1px';
      link.style.borderRadius = '2px';
      return; // no need to GSB check
    }

    // Step 2: GSB check (queued/throttled)
    ...
    new MutationObserver(mutations => {
      ...
      debounceTimer = setTimeout(() => {
        for (const node of pendingNodes) {
          if (node.tagName === 'A' && node.href) { attachLink(node); addBadgeToLink(node); }
          attachAll(node);
          badgeAll(node);
        }
        pendingNodes.clear();
