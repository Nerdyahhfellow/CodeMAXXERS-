...
async function checkGoogleSafeBrowsing(url) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.warn("[Shield] GSB API returned", response.status, body);
      return { threat: false, reason: null };
    }
...
// ── In-memory state ──────────────────────────────────────────────────────────
// Simple Maps — reliable and synchronous. A keepalive alarm prevents the
// service worker from sleeping and losing this state.

const proceedTabExpiry = new Map(); // tabId -> expiry ms

function allowTabProceed(tabId) {
  proceedTabExpiry.set(tabId, Date.now() + 10000);
  console.log("[Shield] allowTabProceed: tabId", tabId);
}
...
const tabUrlHistory = new Map(); // tabId -> { current, prev }
...
// Keep the service worker alive so in-memory state is never lost
chrome.alarms.create("shield-keepalive", { periodInMinutes: 0.4 });
chrome.alarms.onAlarm.addListener(() => { /* keepalive ping */ });

// Persistent GSB cache for link scanning — keyed by URL, never expires during session
const scanCache = new Map(); // url -> { unsafe, threats }
...
chrome.webNavigation.onBeforeNavigate.addListener((details) => {
  ...
  if (details.frameId === 0) {
    chrome.tabs.get(details.tabId, (tab) => {
      if (chrome.runtime.lastError) return;
      if (tab && tab.url) recordNavigation(details.tabId, tab.url); // fire-and-forget async
    });
  }
...
// Clean up tab session data when tab is closed
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  if (msg.type === "PROCEED_ANYWAY") {
    ...
    // 1. Persist proceed whitelist to storage.session (survives service worker sleep)
  ...
  if (msg.type === "SCAN_LINK") {
    ...
    if (local.threat) {
      ...
      const result = { unsafe: true, threats: [{ label: labelMap[local.reason] || 'Threat Detected', type: local.reason }] };
    ...
    checkGoogleSafeBrowsing(url).then(gsbResult => {
      ...
      const labelMap = { malware: 'Malware', phishing: 'Phishing', 'unwanted-software': 'Unwanted Software', 'harmful-app': 'Harmful Application' };
      const result = gsbResult.threat
        ? { unsafe: true, threats: [{ label: labelMap[gsbResult.reason] || 'Threat Detected', type: gsbResult.reason }] }
        : { unsafe: false, threats: [] };
      scanCache.set(url, result);
      sendResponse(result);
